...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const userQuery = `
      ...
      LEFT JOIN healthworker_profiles p ON u.id = p.user_id 
      WHERE (u.email = ? OR u.employee_id = ?) AND u.role = 'healthworker'
    `;
    ...
    return res.status(200).json({
      ...
      user: {
        ...
        phone: user.phone,
        email: user.email,
        name: user.name || 'Health Worker'
      }
    });

  } catch (error) {
    console.error('Health worker login error:', error);
